...
def NonMaxSuppression(bboxes, scores, threshold=0.5):
    ...
    areas = torch.mul((x_coordinate_2 - x_coordinate_1),
    ...
    while len(ordered_indices) != 0:
        ...
        new_x_coordinate_1 = torch.index_select(
            x_coordinate_1, dim=0, index=ordered_indices)
        new_x_coordinate_2 = torch.index_select(
            x_coordinate_2, dim=0, index=ordered_indices)
        new_y_coordinate_1 = torch.index_select(
            y_coordinate_1, dim=0, index=ordered_indices)
        new_y_coordinate_2 = torch.index_select(
            y_coordinate_2, dim=0, index=ordered_indices)

        new_x_coordinate_1 = torch.max(
            new_x_coordinate_1, x_coordinate_1[highest_score_index])
        new_x_coordinate_2 = torch.min(
            new_x_coordinate_2, x_coordinate_2[highest_score_index])
        new_y_coordinate_1 = torch.max(
            new_y_coordinate_1, y_coordinate_1[highest_score_index])
        new_y_coordinate_2 = torch.min(
            new_y_coordinate_2, y_coordinate_2[highest_score_index])

        height = torch.clamp(
            (new_y_coordinate_2 - new_y_coordinate_1), min=0.0)
        width = torch.clamp((new_x_coordinate_2 - new_x_coordinate_1), min=0.0)

        remaining_areas = torch.index_select(
            areas, dim=0, index=ordered_indices)
        intersection_area = height * width
        union = torch.add(
            torch.sub(remaining_areas, intersection_area), areas[highest_score_index])
...
"""
 ...
 - CITATION: some of this code implementation belong to:
...
def mAP_score(detected_boxes, y_pred, scores, gt_boxes, y_true, num_classes, device):
    ...
    true_class_boxes_detected = torch.zeros((y_true.size(0)),
    ...
    for c in range(1, num_classes):
        ...
        detected_class_scores, indices = torch.sort(
            detected_class_scores, dim=0, descending=True)
        detected_class_images = detected_class_images[indices]
        detected_class_boxes = detected_class_boxes[indices]

        # In the order of decreasing scores, check if true or false positive
        true_positives = torch.zeros((num_class_detections), dtype=torch.float).to(
            device)  # (n_class_detections)
        false_positives = torch.zeros((num_class_detections), dtype=torch.float).to(
            device)  # (n_class_detections)
        ...
        for d in range(num_class_detections):
            ...
            object_boxes = true_class_boxes[true_class_images == image]
            ...
            overlaps = calculateIoU(detection_box, object_boxes)
            max_overlap, index = torch.max(
                overlaps.squeeze(0), dim=0)  # (), () - scalars

            # 'ind' is the index of the object in these image-level tensors 'object_boxes', 'object_difficulties'
            # In the original class-level tensors 'true_class_boxes', etc., 'ind' corresponds to object with index...
            original_index = torch.LongTensor(range(true_class_boxes.size(0)))[
                true_class_images == image][index]
            ...
            if max_overlap.item() > 0.5:
                ...
                if true_class_boxes_detected[original_index] == 0:
                    ...
                    true_class_boxes_detected[original_index] = 1
        ...
        cumul_recall = cumul_true_positives / \
            num_class_detections  # (n_class_detections)
